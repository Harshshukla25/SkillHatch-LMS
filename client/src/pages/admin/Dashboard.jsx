...
const Dashboard = () => {
  const { user } = useSelector((state) => state.auth); // Logged-in instructor
  ...
  // Filter purchases belonging to courses created by this instructor
  const instructorPurchases = purchasedCourse.filter(
    (purchase) => purchase?.courseId?.creator?._id === instructorId
  );

  const totalRevenue = instructorPurchases.reduce((acc, curr) => acc + (curr.amount || 0), 0);
  const totalSales = instructorPurchases.length;

  const courseData = instructorPurchases.map((purchase) => ({
